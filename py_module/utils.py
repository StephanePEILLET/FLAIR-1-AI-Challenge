...
def load_data(paths_data, val_percent=0.8, use_metadata=True):
    def _gather_data(
        domains, paths_data: dict, use_metadata: bool, test_set: bool
    ) -> dict:
        #### return data paths
        ...
        for domain in domains:
            for area in os.listdir(
                Path(
                    paths_data[
                        "path_aerial_" + ["train" if test_set == False else "test"][0]
                    ],
                    domain,
                )
            ):
                    ...
                    list(
                        get_data_paths(
                            Path(
                                paths_data[
                                    "path_aerial_"
                                    + ["train" if test_set == False else "test"][0]
                                ],
                                domain,
                                area,
                            ),
    ...
    dict_train = _gather_data(
        train_domains, paths_data, use_metadata=use_metadata, test_set=False
    )
    dict_val = _gather_data(
        val_domains, paths_data, use_metadata=use_metadata, test_set=False
    )

    path_test = Path(paths_data["path_aerial_test"])
    test_domains = os.listdir(path_test)

    dict_test = _gather_data(
        test_domains, paths_data, use_metadata=use_metadata, test_set=True
    )
...
def print_recap(config, dict_train, dict_val, dict_test):
    ...
    for info, val in zip(
        ["use weights", "use metadata", "use augmentation"],
        [config["use_weights"], config["use_metadata"], config["use_augmentation"]],
    ):
